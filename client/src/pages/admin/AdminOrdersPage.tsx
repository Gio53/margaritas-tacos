...
export default function AdminOrdersPage() {
  ...
  // Unlock AudioContext on first click so new-order alarm can play (browser policy)
  useEffect(() => {
    const unlock = () => {
      try {
  ...
  const openTicketPrintWindow = (order: PlacedOrder) => {
    ...
    if (w) {
      ...
      setTimeout(() => {
        w.focus();
        const closeWin = () => {
          w.removeEventListener("afterprint", closeWin);
          w.close();
        };
        w.addEventListener("afterprint", closeWin);
        w.print();
      }, 200);
    }
  };
  ...
  useEffect(() => {
    ...
    const newIds = Array.from(currentIds).filter((id) => !knownOrderIdsRef.current.has(id));
  ...
  // Stop the new-order alarm when user presses Spacebar
  useEffect(() => {
  ...
  return (
    <>
      <div className="flex-1 p-4 md:p-6 max-w-3xl mx-auto w-full" style={{ fontFamily: "'Lato', sans-serif" }}>
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {useApi && (
              <span className="text-xs px-2 py-1 rounded border" style={{ borderColor: "rgba(44,24,16,0.2)", color: "#6B7280" }}>
                Synced
              </span>
            )}
            {API_BASE && (
